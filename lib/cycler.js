...
class Cycler {
    ...
    constructor (proxies) {
        ...
        for (let i = 0; i < proxies.length; i++) {
            ...
            this.add(proxy.host, proxy.port, proxy.protocol);
        }
    }
     ...
     */
    add (host, port, protocol) {
        const portCopy = parseInt(port);

        const proxy = new HTTPProxy(host, portCopy, protocol);
        const exists = this._exists(proxy.host, proxy.port, protocol);
        if (exists !== false) {
     ...
     */
    remove (host, port, protocol) {
        const portCopy = parseInt(port);

        const exists = this._exists(host, portCopy, protocol);
        if (exists !== false) {
     ...
     */
    use (host, port, protocol, target, date) {
        let proxy;

        const exists = this._exists(host, port, protocol);
        if (exists !== false) {
            proxy = this.proxies[exists];
        } else {
            proxy = this.add(host, port, protocol);
        }
     ...
     */
    _exists (host, port, protocol) {
        for (let i = 0; i < this.proxies.length; i++) {
            const proxy = this.proxies[i];
            if (proxy.host === host && proxy.port === port && proxy.protocol === protocol) {
                return i;
            }
        }
        return false;
