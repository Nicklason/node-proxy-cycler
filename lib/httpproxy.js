...
class HTTPProxy {
     ...
     */
    constructor (host, port, auth = null) {
        const portCopy = parseInt(port);

        HTTPProxy.validateProxy(host, portCopy, auth);
     ...
     */
    url () {
        return `http://${this.auth === null ? '' : this.auth + '@'}${this.host}:${this.port}`;
    }
     ...
     */
    static validateProxy (host, port, auth) {
        if (host === undefined || port === undefined || auth === undefined) {
            throw new Error('Missing host, port or auth');
